...
import { ShieldCheck, Trash2, Search, AlertCircle } from 'lucide-react';
...
export default function AdminView() {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      setMsg(`User "${userToDelete.name}" deleted successfully.`);
  ...
  return (
    <div className="space-y-6">
      ...
      {msg && (
        <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold text-center animate-[fadeIn_0.2s_ease-out]">
          {msg}
        </div>
      )}

      {error && (
        <div className="flex items-center justify-center gap-2 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-bold text-center animate-[shakeIn_0.3s_ease-out]">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-slate-900/60 border border-slate-800 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 focus:outline-none focus:border-purple-500 transition-all duration-200"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                  ...
                  <tr key={`skeleton-${i}`}>
                    {Array.from({ length: 6 }).map((__, j) => (
                      <td key={j} className="py-3 px-4">
                        <div className="h-3 rounded bg-slate-800/80 animate-pulse" style={{ width: `${50 + (j * 7) % 40}%` }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                filteredUsers.map(user => (
                  <tr key={user.id} className="hover:bg-slate-800/40 transition-all duration-200">
